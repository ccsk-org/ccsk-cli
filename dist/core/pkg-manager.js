...
async function isOwnedBy(spec, pkg) {
    if (!(await binExists(spec.bin)))
        return false;
...
export async function detectCcskOwner() {
    for (const spec of MANAGERS) {
        if (await isOwnedBy(spec, CCSK_PACKAGE_NAME))
            return spec;
    }
    return null;
}
 ...
 */
export async function removeCcskGlobally() {
    const attempted = [];
    const tryRemove = async (spec) => {
        if (!(await binExists(spec.bin)))
            return false;
