...
function paint256(code: number, s: string): string {
...
function shimmerLabel(label: string, tick: number): string {
...
export async function withShimmer<T>(label: string, fn: () => Promise<T>): Promise<T> {
  if (!isInteractive()) {
  ...
  const finish = (icon: string, color: (s: string) => string, suffix: string): void => {
