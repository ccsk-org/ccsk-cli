...
export type AuthMethod = 'ssh' | 'gh' | 'none';
...
async function checkSshAccess(): Promise<{ ok: boolean; username?: string }> {
  try {
...
async function checkGhAuth(): Promise<{ ok: boolean; username?: string }> {
  if (!binExists('gh')) {
    return { ok: false };
  }
  try {
...
export function getCloneUrl(repo: string, method: AuthMethod): string {
  if (method === 'ssh') {
...
export async function ensureGitHubAuth(): Promise<AuthStatus> {
