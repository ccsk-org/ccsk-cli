 ...
 */
export async function addContextModeMcp(): Promise<StepResult> {
  ...
  const { exitCode, stderr } = await execa(
    'claude',
    ['mcp', 'add', 'context-mode', '--', 'npx', '-y', 'context-mode'],
    { reject: false },
  );
 ...
 */
export function printContextModeInstructions(): StepResult {
